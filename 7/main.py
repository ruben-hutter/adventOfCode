...
def compute_dirsize(dirname: str):
    ...
    for i in subdirs[dirname]: # go through all the subdirectories
        if i in subdirs:
            dirsize += compute_dirsize(i) # looping the subdirectory size addition until we no longer find the subdirectories
    return dirsize
